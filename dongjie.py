...
#先跑两个epoch
model.fit([input_train_ids, input_train_mask, input_train_tokentype, X_train],y_train,batch_size=config.batch_size,epochs=2,verbose=1)
...
evl=evaluator.Evaluator(config,config.out_dir,config.batch_size,input_test_ids,input_test_mask,input_test_tokentype,X_test,y_test)
...
for ii in range(config.epochs):
    t0 = time()
    train_history=model.fit([input_train_ids, input_train_mask, input_train_tokentype, X_train],y_train,batch_size=config.batch_size,epochs=1,verbose=1)
